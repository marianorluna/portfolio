...
import { SCENE_COLORS, type SceneTheme } from "@/config/scene-theme";
...
export function setupScene(theme: SceneTheme = "dark"): {
  scene:              THREE.Scene;
  lights:             SceneLights;
  ...
  const {
    ...
    horizonColor: colors.horizonLine,
  });
  scene.add(gridMesh);
  ...
  const dir1 = new THREE.DirectionalLight(colors.dirLight1.color, colors.dirLight1.intensity);
  dir1.position.set(20, 40, -20);
  scene.add(dir1);

  const dir2 = new THREE.DirectionalLight(colors.dirLight2.color, colors.dirLight2.intensity);
  dir2.position.set(-20, 20, 20);
  scene.add(dir2);

  const dir3 = new THREE.DirectionalLight(colors.dirLight3.color, colors.dirLight3.intensity);
  dir3.position.set(20, 20, -20);
  scene.add(dir3);

  const fillA = new THREE.DirectionalLight(colors.fillA.color, colors.fillA.intensity);
  fillA.position.set(-45, 32, -50);
  scene.add(fillA);

  const fillB = new THREE.DirectionalLight(colors.fillB.color, colors.fillB.intensity);
